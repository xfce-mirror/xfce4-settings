                 ...
                 gint ui_scale_factor)
{
    ...
    gchar *profile_name = xfconf_channel_get_string (x11_channel, profile_root_prop, NULL);
    if (profile_name != NULL)
    {
        xfconf_channel_set_string (wl_channel, profile_root_prop, profile_name);
        g_free (profile_name);
    }
    ...
    while (g_hash_table_iter_next (&iter, (gpointer) &property_name, NULL))
    {
        ...
        {
            const gchar *monitor_name_slash = g_strrstr_len (property_name, last - property_name, "/");
            if (monitor_name_slash != NULL && monitor_name_slash > property_name)
            {
                gchar *monitor_name = g_strndup (monitor_name_slash + 1, last - monitor_name_slash - 1);
                monitors = g_list_prepend (monitors, monitor_name);
            }
        }
    }
    ...
    {
        const gchar *monitor_name = mp->data;

        for (gsize i = 0; i < G_N_ELEMENTS (profile_copy_verbatim); i++)
        {
            gchar *prop = g_strdup_printf ("%s/%s/%s", profile_root_prop, monitor_name, profile_copy_verbatim[i]);
        ...
        gchar *scale_prop = g_strdup_printf ("%s/%s/Scale", profile_root_prop, monitor_name);
        const GValue *scale_value = g_hash_table_lookup (properties, scale_prop);
        gdouble x11_scale = MAX (0.01, scale_value != NULL && G_VALUE_HOLDS_DOUBLE (scale_value) ? g_value_get_double (scale_value) : 1.0);
        gdouble wl_scale = (gdouble) ui_scale_factor / x11_scale;
        xfconf_channel_set_double (wl_channel, scale_prop, MAX (wl_scale, 0.01));
        g_free (scale_prop);

        gchar *posx_prop = g_strdup_printf ("%s/%s/Position/X", profile_root_prop, monitor_name);
        const GValue *posx_value = g_hash_table_lookup (properties, posx_prop);
        gint x11_posx = posx_value != NULL && G_VALUE_HOLDS_INT (posx_value) ? g_value_get_int (posx_value) : 0;
        gint wl_posx = x11_posx / ui_scale_factor;
        xfconf_channel_set_int (wl_channel, posx_prop, wl_posx);
        g_free (posx_prop);

        gchar *posy_prop = g_strdup_printf ("%s/%s/Position/Y", profile_root_prop, monitor_name);
        const GValue *posy_value = g_hash_table_lookup (properties, posy_prop);
        gint x11_posy = posy_value != NULL && G_VALUE_HOLDS_INT (posy_value) ? g_value_get_int (posy_value) : 0;
