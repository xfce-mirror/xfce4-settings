                                    ...
                                    RotationFlags rotation)
{
    ...
    lp = g_list_nth (get_instance_private (settings)->outputs, output_id);
    if (lp != NULL)
    {
        XfceOutput *output = lp->data;
                                ...
                                guint mode_id)
{
    ...
    lp = g_list_nth (get_instance_private (settings)->outputs, output_id);
    if (lp != NULL)
        XFCE_DISPLAY_SETTINGS_GET_CLASS (settings)->update_output_mode (settings, lp->data, mode_id);
}
...
xfce_display_settings_mirror (XfceDisplaySettings *settings)
{
    ...
    for (GList *lp = get_instance_private (settings)->outputs; lp != NULL; lp = lp->next, n++)
