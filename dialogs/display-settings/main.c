                                         ...
                                         GtkBuilder      *builder)
{
    ...
    if (gtk_toggle_button_get_active (togglebutton))
    {
        ...
        modes = xfce_randr_clonable_modes (xfce_randr);
...
display_setting_mirror_displays_populate (GtkBuilder *builder)
{
    ...
    gtk_widget_set_sensitive (GTK_WIDGET (check), modes != NULL);
    if (modes == NULL)
    {
...
get_mirrored_configuration (void)
{
    ...
    for (n = 0; n < xfce_randr->noutput; n++)
    {
        ...
        cloned &= (xfce_randr->mode[n] == modes[n] &&
                   xfce_randr->mirrored[n]);
        mirrored = xfce_randr->mirrored[n];
...
display_settings_show_minimal_dialog (GdkDisplay *display)
{
                                     ...
                                     minimal_display_dialog_ui_length, &error) != 0)
    {
        ...
        if (xfce_randr->noutput > 1)
        {
            ...
            gtk_widget_set_sensitive (GTK_WIDGET (mirror_displays), modes != NULL);
            gtk_widget_set_sensitive (GTK_WIDGET (mirror_displays_label), modes != NULL);
            g_free (modes);
