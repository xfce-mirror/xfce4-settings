...
// The device store owns a reference, so the caller gets a borrowed pointer that
// stays valid for as long as the device remains in the combobox.
static XfceDevice *
...
mouse_settings_touchscreen_populate_monitors (GtkBuilder *builder)
{
    ...
    XfceDevice *device = mouse_settings_device_get_selected (builder);
    if (device != NULL)
    {
                                           ...
                                           XfceDevice *device)
{
    ...
    if (XFCE_IS_DEVICE_X11 (device))
    {
        mode = xfce_device_x11_get_wacom_mode (XFCE_DEVICE_X11 (device));
    }
#endif

    GObject *object = gtk_builder_get_object (builder, "wacom-mode-label");
    gtk_widget_set_visible (GTK_WIDGET (object), mode != -1);

    object = gtk_builder_get_object (builder, "wacom-mode");
    gtk_widget_set_visible (GTK_WIDGET (object), mode != -1);
    gtk_combo_box_set_active (GTK_COMBO_BOX (object), mode == -1 ? 1 : mode);
}
