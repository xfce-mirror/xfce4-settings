...
{
    gboolean available;
    gboolean button_areas;
    gboolean click_finger;
} click_method_t;
#endif
...
mouse_settings_get_libinput_click_method (Display        *xdisplay,
                                          XDevice        *device,
                                          const gchar    *prop_name,
                                          click_method_t *click_method)
{
    propdata_t pdata[2];

    if (mouse_settings_get_device_prop (xdisplay, device, prop_name, XA_INTEGER, 2, &pdata[0]))
    {
        click_method->available = TRUE;
        click_method->button_areas = pdata[0].c;
        click_method->click_finger = pdata[1].c;
...
mouse_settings_device_selection_changed (GtkBuilder *builder)
{
    ...
    click_method_t     libinput_click_methods_available = {FALSE};
    click_method_t     libinput_click_method = {FALSE};
#endif /* HAVE_LIBINPUT */
    Atom               synaptics_prop;
    Atom               wacom_prop;
        ...
        if (gdk_x11_display_error_trap_pop (gdk_display_get_default ()) == 0 && props != NULL)
        {
            for (i = 0; i < nprops; i++)
            {
                ...
                else if (props[i] == libinput_click_method_prop)
                {
                    ...
                    success = mouse_settings_get_libinput_click_method (xdisplay, device,
                                                                        LIBINPUT_PROP_CLICK_METHODS_AVAILABLE,
                                                                        &libinput_click_methods_available);

                    if (success)
                    {
                        mouse_settings_get_libinput_click_method (xdisplay, device,
    ...
    if (is_synaptics)
    {
        ...
        if (is_libinput)
        {
            gtk_widget_set_sensitive (GTK_WIDGET (object), libinput_click_method.available);
            /*
             * Both values can't be enabled at a time according to documentation.
             * When I set both values the driver behaves like click_finger, so
             * prioritize the click_finger value.
             */
            if (libinput_click_method.click_finger)
                gtk_combo_box_set_active (GTK_COMBO_BOX (object), 2);
            else if (libinput_click_method.button_areas)
        ...
        if (is_libinput)
        {
            ...
            if (gtk_tree_model_iter_nth_child (GTK_TREE_MODEL (object), &iter, NULL, 1))
                gtk_list_store_set (GTK_LIST_STORE (object), &iter, 1, libinput_click_methods_available.button_areas, -1);

            if (gtk_tree_model_iter_nth_child (GTK_TREE_MODEL (object), &iter, NULL, 2))
                gtk_list_store_set (GTK_LIST_STORE (object), &iter, 1, libinput_click_methods_available.click_finger, -1);
        }
