                                                   ...
                                                   gpointer user_data)
{
     ...
     * in this case: see
     * - https://gitlab.gnome.org/GNOME/glib/-/issues/104
     * - https://gitlab.gnome.org/GNOME/glib/-/issues/2927
    ...
    /* avoid cycling if possible: it will probably trigger monitor_changed() despite this
     * blocking because changes on disk end after a certain delay, but the cycle will stop
     * there, so better not to complicate the code unnecessarily */
    if (helper->monitor != NULL)
