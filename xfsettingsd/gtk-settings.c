...
xfce_gtk_settings_helper_init (XfceGtkSettingsHelper *helper)
{
    ...
    if (settings == NULL)
    {
        g_critical ("Failed to get GtkSettings instance");
        return;
    }

    /* at least gsettings-desktop-schemas should be found since it is required */
    source = g_settings_schema_source_get_default ();
    if (source == NULL)
    {
        g_critical ("No GSettings schema could be found");
        return;
    }
    ...
    for (guint i = 0; i < G_N_ELEMENTS (translations); i++)
    {
        ...
        {
            GParamSpec *pspec = g_object_class_find_property (G_OBJECT_GET_CLASS (settings), translations[i].gtksetting);
            if (pspec == NULL)
    ...
    /* remaining properties: synchronized via gtk-modules */
                                                   ...
                                                   XfceGtkSettingsHelper *helper)
{
    ...
    pspec = g_object_class_find_property (G_OBJECT_GET_CLASS (settings), data->gtksetting);
    default_value = g_param_spec_get_default_value (pspec);

    /* Xfconf property removed => GtkSettings property reset */
    if (G_VALUE_TYPE (value) == G_TYPE_INVALID)
        value = default_value;

    /* unlike GSettings the type of Xfconf properties can change or they can be newly created */
    if (! g_value_type_transformable (G_VALUE_TYPE (value), G_VALUE_TYPE (default_value)))
    {
