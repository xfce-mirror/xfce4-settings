...
static gboolean
should_sync_property (const gchar *property)
{
    for (gsize i = 0; i < G_N_ELEMENTS (sync_properties); ++i)
        if (g_strcmp0 (property, sync_properties[i]) == 0)
            return TRUE;
    return FALSE;
}
...
property_changed (XfconfChannel *channel,
                  ...
                  gpointer data)
{
    if (should_sync_property (property))
    {
