...
If you want to support the in-process method, add
`X-XfcePluggableInProcess=true` to the `.desktop` file.
...
should be named the same as the desktop ID.  That is, if your `.desktop`
file is named `xfce-backdrop-settings.desktop`, the shared library
should be named `libxfce-backdrop-settings.so`.
...
exec -a "$0" xfce4-settings-manager --standalone --dialog $DIALOG_NAME
```
...
(If you don't want to depend on `bash`, you can change the shebang to
`/bin/sh` and remove the `-a "$0"` option from `exec`.  The `-a` option
just makes it easier to distinguish different instances of
`xfce4-settings-manager` running in `ps` output.)
